...
class TestForwardRemotePort(TestCase):
    def setUp(self):
    ...
    def test_forward_remote_port_logs_debug_before_context(self):
        with forward_remote_port('127.0.0.1', 22):
            self.assertTrue(self.log.debug.called)

    def test_forward_remote_port_runs_forward_command_before_context(self):
        ...
        with forward_remote_port('127.0.0.1', 22):
            self.run_detached_command.assert_called_once_with(
                ...
                '-L 22:localhost:22 sleep 600'
            )

    def test_forward_remote_port_gives_the_tunnel_some_time_to_establish(self):
        self.assertFalse(self.sleep.called)

        with forward_remote_port('127.0.0.1', 22):
            self.sleep.assert_called_once_with(1)

    def test_forward_remote_port_kills_forward_after_context(self):
        with forward_remote_port('127.0.0.1', 22):
