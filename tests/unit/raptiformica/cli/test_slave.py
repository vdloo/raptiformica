...
class TestSlave(TestCase):
    def setUp(self):
        ...
        self.parse_slave_arguments.return_value = Mock(host='1.2.3.4', port=22, no_assimilate=False)
    ...
    def test_slave_slaves_machines(self):
        ...
        self.slave_machine.assert_called_once_with(
            '1.2.3.4', port=22, assimilate=True
        )

    def test_slave_does_not_assimilate_machine_when_no_assimilate_is_passed(self):
        self.parse_slave_arguments.return_value = Mock(host='1.2.3.4', port=22, no_assimilate=True)

        slave()

        self.slave_machine.assert_called_once_with(
            '1.2.3.4', port=22, assimilate=False
        )
